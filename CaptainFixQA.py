...
#
#     try:
#         response = chain.invoke({"html": html_content, "test_cases": test_cases,"url":CONFIG.get("url",
#                                                                                                  "file:////Users/raniaburaia/Desktop/certificate/Captain-Fix/Search engine.html")})
#     except Exception as e:
#         raise RuntimeError(f"Failed to invoke chain: {e}")
#
#     if hasattr(response, "content"):
...
#
    ...
    if len(sys.argv) > 1:
        ...
        model='gpt-3.5-turbo',
        temperature=0.1,
        openai_api_key="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"
    )
    ...
    try:
    # בדיק 1: הכנסת טקסת לחיפוס ולחיצה על כפתור חיפוש
        search_bar = driver.find_element(By.ID,"search-bar")
        search_bar.send_keys("חפש משהו...")
        search_button = driver.find_element(By.ID,"search-btn")
        search_button.send_keys(Keys.RETURN)
        time.sleep(2)  # המתנה לטעינת התוצאות
    except Exception as e:
